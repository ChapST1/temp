...
export function ReadingListItem ({ book, removeToReadingList }) {
  ...
  return (
    <div key={id} className='w-full h-[200px] border-[4px] border-[#f9fafb] overflow-hidden p-1 rounded-md relative group' id={id}>
      <img
        src={bookCover}
        alt={`cover del libro: ${bookTitle}`}
        className='w-full h-full rounded-md object-cover object-top'
      />
      <div className=' h-[60px] duration-200 translate-y-[100%] absolute bottom-0 left-0 w-full flex items-center justify-center bg-gradient-to-t from-[#f9fafb] via-[#f9fafbd9] to-[#f9fafb09] group-hover:translate-y-[0%]'>
